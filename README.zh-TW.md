...
- **邊緣快取** — 建一個 KV namespace 綁成 `LINKS_KV`（見 `wrangler.toml.example`），轉址會先讀 KV，降低 D1 讀取與延遲。改連結後仍會在 `CACHE_TTL`（預設 60 秒）內生效。沒綁＝永遠讀 D1（即時，現有行為）。
- **自動清理** — 設 `RETENTION_DAYS` 並啟用 `[triggers]` 排程，每天自動刪掉超過 N 天的點擊。不設＝永久保留。

> **從舊版升級？** 不重複訪客功能多了一個欄位，對你的 D1 跑一次：
> ```sql
